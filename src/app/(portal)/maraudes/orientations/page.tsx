...
export default function OrientationsPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      <h1 className="text-2xl font-bold mb-6">Orientations</h1>

      <div className="mb-4 flex gap-2 flex-wrap">
        {['', ...Object.keys(statusLabels)].map(s => (
          <button key={s} onClick={() => { setStatusFilter(s); setPage(1); }}
            className={`px-3 py-1 rounded text-sm ${statusFilter === s ? 'bg-green-600 text-white' : 'bg-gray-100'}`}>
          ...
          {referrals.map(r => (
            <div key={r.id} className="bg-white p-4 rounded-lg shadow">
              <div className="flex justify-between items-start">
                <div>
                  <p className="font-medium">{r.beneficiary?.nickname || 'Inconnu'} → {r.structure?.name || r.structureName || 'Structure inconnue'}</p>
