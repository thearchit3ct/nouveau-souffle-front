  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-4 sm:px-6 sm:py-6">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mb-6">
        <h1 className="text-xl sm:text-2xl font-bold">Beneficiaires</h1>
        <Link href="/maraudes/beneficiaires/new" className="bg-green-600 text-white px-4 py-2.5 rounded hover:bg-green-700 text-center text-sm sm:text-base">
          Nouvelle fiche
        </Link>
      </div>
      ...
      <div className="mb-4 flex gap-3 flex-wrap">
        <input type="text" placeholder="Rechercher (surnom, lieu...)" value={search}
          ...
          className="border rounded px-3 py-2.5 w-full sm:w-64 min-h-[44px]" />
        <select value={housingFilter} onChange={e => { setHousingFilter(e.target.value); setPage(1); }}
          className="border rounded px-3 py-2.5 min-h-[44px]">
      ...
      {loading ? <p>Chargement...</p> : (
        <>
          ...
          <div className="md:hidden space-y-3">
            {beneficiaries.map(b => (
              <Link key={b.id} href={`/maraudes/beneficiaires/${b.id}`} className="block bg-white rounded-lg shadow p-4 active:bg-gray-50">
                <div className="flex justify-between items-start mb-1">
                  <span className="font-medium text-green-700">{b.nickname}</span>
                  <span className={`text-xs px-2 py-1 rounded ${b.gdprConsentStatus === 'GIVEN' || b.gdprConsentStatus === 'ORAL' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                    {b.gdprConsentStatus}
                  </span>
                </div>
                <div className="flex gap-3 text-sm text-gray-500">
                  <span>{housingLabels[b.housingStatus] || b.housingStatus}</span>
                  {b.usualLocation && <span>{b.usualLocation}</span>}
                </div>
                <p className="text-xs text-gray-400 mt-1">{b._count?.encounters ?? 0} rencontres</p>
              </Link>
            ))}
            {beneficiaries.length === 0 && (
              <p className="text-center text-gray-500 py-8">Aucun beneficiaire trouve</p>
            )}
          </div>
        </>
      )}
